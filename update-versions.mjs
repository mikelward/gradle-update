...
// that mark a VARIANT of a stable release (guava's -android/-jre, "-release",
// "-ga", "-final") are deliberately not here.
const UNSTABLE_TOKEN =
  /^(alpha|beta|rc|cr|m|milestone|ea|eap|dev|preview|pre|snapshot|snap|nightly|a|b)\d*$/i;
...
export const compareVersions = (a, b) => {
  ...
  for (let i = 0; i < len; i++) {
    const x = ta[i];
    const y = tb[i];
    if (x === undefined) return UNSTABLE_TOKEN.test(y) ? 1 : -1;
    if (y === undefined) return UNSTABLE_TOKEN.test(x) ? -1 : 1;
    const nx = /^\d+$/.test(x);
    const ny = /^\d+$/.test(y);
    if (nx && ny) {
...
export const variantOf = (v) =>
  tokenize(v)
    .filter((t) => t !== "" && !/^\d+$/.test(t) && !UNSTABLE_TOKEN.test(t))
    .join("-");
...
// How many candidates the cooldown walks before giving up and holding the
// current version. Bounds HTTP requests; a dependency that shipped this many
// releases inside one cooldown window can wait a week.
const COOLDOWN_CANDIDATES = 5;
...
export const decideUpdate = async (
  ...
  const stable = shared.filter(isStable);
  ...
  for (const candidate of inMajor.slice(0, COOLDOWN_CANDIDATES)) {
